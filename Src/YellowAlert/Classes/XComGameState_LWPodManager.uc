...
//---------------------------------------------------------------------------------------
class XComGameState_LWPodManager extends XComGameState_BaseObject config(LW_PodManager);
...
// Keep track of the mission complete turn (or in some cases VIP acquired)
var int ObjectiveCompleteTurn;
...
function UpdatePod(XComGameState NewGameState, XComGameState_AIGroup GroupState)
{
	...
	if (PodJ == none)
	{
		PodJ = FindPodJobForPod(GroupState);
	}
	
	if (PodJ != none)
	{	
		PodJ = XComGameState_LWPodJob(NewGameState.ModifyStateObject(class'XComGameState_LWPodJob', PodJ.ObjectID));
		if (!PodJ.ProcessTurn(self, NewGameState))
		{
...
function UpdateXComPosition()
{
	...
	else
	{
		...
		// Use the ojective location when the mission is completed and no one has eyes on XCom, unless there is newer alert data
		// And noone has had eyes on XCom since the objective was complete
		if (ObjectiveCompleteTurn >= BestTurn && ObjectiveCompleteTurn >= LastKnownXComPositionTurn)
		{
	...
	while (JobList.Length > 0 && UnassignedPods.Length > 0)
	{
		...
		for (JobListIdx = 0; JobListIdx < JobList.Length; ++JobListIdx)
		{
			if (PodJobIsValidForPod(JobList[JobListIdx], Group))
			{
				...
				JobAssign.GroupID = Group.ObjectID;
				JobAssign.Job = JobList[JobListIdx];
				AssignedJobs.AddItem(JobAssign);
				if (!Job.Unlimited)
				{
