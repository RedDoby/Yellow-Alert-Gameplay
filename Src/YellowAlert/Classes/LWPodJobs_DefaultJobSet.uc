...
static function array<X2DataTemplate> CreateTemplates()
{
	...
	`Log("  >> LWPodJobs_DefaultJobSet.CreateTemplates()");
...
// Guard/Defend pods will patrol around a small 'encounter zone' near the Evac Zone. Just keeps them nearby but
// not completely stationary.
function Vector EvacZoneLocationCallback(XComGameState_LWPodJob Job, XComGameState NewGameState)
{
	...
	// Choose a new location nearby (+/- GUARD_PATROL_DISTANCE tiles as a rectangle surrounding the evac zone)
	NewLocation = `XWORLD.GetPositionFromTileCoordinates(`LWPODMGR.EvacZoneLocation);
	switch(`SYNC_RAND(4))
	{
		case 0:
			// "NW" corner (-X, +Y)
